...
const watchTree = (rootPath, changed) => {
	const prevContents = {};
	let watchers = [];
	let allDependencyPaths = Array.from(
		new Set(flatten(resolveDependencies(undefined, rootPath)))
	).sort();
	...
	const handleWatch = (filePath, eventType, filename) => {
		...
		if (prevContents[filePath] !== contents) {
			...
			const updatedDepencyPaths = Array.from(
				new Set(flatten(resolveDependencies(undefined, rootPath)))
			);
			const depsChange =
				JSON.stringify(updatedDepencyPaths.sort()) !== JSON.stringify(allDependencyPaths);
...
module.exports = function makeFsSideEffects() {
	...
	const fsSource = () => {
		...
		const watch$ = scriptDataFromCB.stream.debounce(400).skipRepeats().multicast();
		return most.mergeArray([fs$, watch$]);
	};
