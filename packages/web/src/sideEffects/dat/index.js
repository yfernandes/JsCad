...
const makeDatSideEffect = async (params) => {
	...
	const sink = (commands$) => {
		...
		commands$.forEach((command) => {
			...
			if (!enabled) {
				commandResponses.callback({
					type,
					id,
					error: new Error("Dat archives not supported in this environment!"),
				});
				return;
			}
			...
			const unhandled = () => {
				commandResponses.callback({
					type,
					id,
					error: new Error(`Dat: no handler found for command ${type}`),
				});
			};
			const read = () => {
				urls.map(async (url) => {
					...
					const transformed = fileContents
						.filter((x) => x !== undefined)
						.map((f) => {
							const entry = {
								name: f.name,
								ext: getFileExtensionFromString(f.name),
								source: f.content,
								fullPath: `/${rootFolder}/` + f.fullPath,
							};
							return entry;
						});
					hiearchyRoot = [
