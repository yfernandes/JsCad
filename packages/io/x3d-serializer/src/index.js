...
const convertPath2 = (object, options) => {
	...
	const shape = [
		...
		convertPolyline2D(poly2.create(points), options),
	];
	if (object.color) {
...
const convertGeom2 = (object, options) => {
	...
	outlines.forEach((outline) => {
		...
		const shape = [
			"Shape",
			shapeAttributes(object),
			convertPolyline2D(poly2.create(outline), options),
		];
		if (object.color) {
			shape.push(convertAppearance(object, "emissiveColor", options));
		}
...
const convertToTriangles = (object, options) => {
	...
	polygons.forEach((poly) => {
		...
		for (let i = poly.vertices.length - 3; i >= 0; i--) {
			const triangle = poly3.create([firstVertex, poly.vertices[i + 1], poly.vertices[i + 2]]);
