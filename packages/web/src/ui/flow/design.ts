...
const actions = ({sources}) => {
	...
	const requestLoadSettings$ = initialize$.map((_) => ({
		sink: "store",
		key: "design",
		type: "read",
	}));
	...
	const requestSaveSettings$ = sources.state
		...
		.thru(
			holdUntil(sources.store.filter((reply) => reply.key === "design" && reply.type === "read"))
		)
	...
	const requestLoadDesignContent$ = most
		.mergeArray([
	...
	// from all sources ...
	const setDesignContent$ = most
		.mergeArray(Object.values(sources).filter((x) => x !== undefined && "source" in x))
		// ... containing loadRemote responses
		.filter(
			(response) =>
				response.id === "loadRemote" &&
				response.type === "read" &&
				!("error" in response) &&
				!("sink" in response)
		)
	...
	const requestWriteCachedGeometry$ = most
		.mergeArray([
			sources.state
				.filter(reducers.isDesignValid)
		...
		.map((payload) =>
			Object.assign({}, {type: "write", id: "cachedGeometry", sink: payload.origin}, payload)
		)
		.multicast();

	const resetDesign$ = most
		.mergeArray([requestLoadDesignContent$.map(({sink}) => sink)])
		// .thru(holdUntil(setDesignSettings$)) // only after FIXME : this does not seem to work
		.thru(withLatestFrom(reducers.resetDesign, sources.state))
		.map((data) => ({type: "resetDesign", state: data, sink: "state"}))
		.multicast();

	const setDesignSolids$ = most
		.mergeArray([
			sources.solidWorker
				.filter(
					(event) =>
						!("error" in event) && event.data instanceof Object && event.data.type === "solids"
				)
				.map((event) => {
	...
	const timeoutGeometryRecompute$ = requestGeometryRecompute$
		.thru(
			delayFromObservable(
				(state) => state.design.solidsTimeOut,
				sources.state.filter(reducers.isDesignValid)
			)
		)
		.thru(withLatestFrom(reducers.timeoutGeometryRecompute, sources.state))
		.map((payload) =>
			Object.assign({}, {state: payload}, {sink: "state", type: "timeOutDesignGeneration"})
		)
		.multicast();
	...
	const parameterDefinitionsFromWorker$ = sources.solidWorker
		.filter(
			(event) => !("error" in event) && event.data instanceof Object && event.data.type === "params"
		)
		.map(({data}) => ({
	...
	const setDesignParameterDefinitions$ = most
		.mergeArray([parameterDefinitionsFromWorker$])
		.skipRepeatsWith(jsonCompare)
	...
	const getControls = () =>
		Array.from(document.getElementById("paramsTable").getElementsByTagName("input"))
			.concat(Array.from(document.getElementById("paramsTable").getElementsByTagName("select")))
			.concat(
				Array.from(document.getElementById("paramsTable").getElementsByClassName("groupTitle"))
			);

	const getInstantUpdate = () => document.getElementById("instantUpdate").checked;

	const parametersFromDom$ = most
		.mergeArray([
			sources.dom
	...
	const parametersFromStore$ = sources.store
		.filter(
			(reply) =>
				reply.key === "design" && reply.type === "read" && reply.data.parameterValues !== undefined
		)
	...
	const validDesignState$ = sources.state.filter(reducers.isDesignValid).multicast();

	const setDesignParameterValues$ = most
		.mergeArray([
			parametersFromDom$,
			parametersFromStore$,
			...
			sources.dom
				.select("#resetDesignToParameterDefaults")
				.events("click")
				.thru(
					withLatestFrom(
						(state, _) => ({parameterValues: state.design.parameterDefaults, origin: "reset"}),
						sources.state
					)
				),
		])
		...
		.thru(
			holdUntil(
				validDesignState$.filter((state) => {
					const hasParamDefinitions =
						state.design && Object.keys(state.design.parameterDefinitions).length > 0;
	...
	const reportErrorsFromWorker$ = most
		.mergeArray([errorsFromWorker$])
		.skipRepeatsWith(jsonCompare)
		.tap((x) => console.log("errors", x));

	// ui/toggles
	const toggleAutoReload$ = most
		.mergeArray([
			sources.dom
	...
	const toggleInstantUpdate$ = most
		.mergeArray([
			sources.dom
				.select("#instantUpdate")
				.events("click")
	...
	const toggleVTreeMode$ = most
		.mergeArray([
			sources.dom
				.select("#toggleVtreeMode")
				.events("click")
				.map((event) => event.target.checked),
		])
	...
	const setSolidsTimeout$ = most
		.mergeArray([
			sources.dom
				.select("#solidsTimeout")
				.events("change")
