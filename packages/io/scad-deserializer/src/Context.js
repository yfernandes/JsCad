...
var functionNameLookup = {
	cos: function (degree) {
		if (_.isUndefined(degree) || _.isNaN(degree)) {
			return undefined;
		}
	...
	atan2: function (x, y) {
		if (_.isUndefined(x) || _.isNaN(x) || _.isUndefined(y) || _.isNaN(y)) {
			return undefined;
		}
	...
	log: function () {
		if (arguments.length == 2) {
			...
			if (_.isUndefined(arguments[0]) || _.isNaN(arguments[0])) {
				return undefined;
			}
	...
	lookup: function () {
		...
		if (vector[0].length != 2) {
			console.log(
				"Lookup arguments are invalid. First vector has incorrect number of values. " +
					p +
					",  " +
					vector
			);
