...
test("align: single object returns geometry unchanged if all axes are none", (t) => {
	...
	t.true(
		comparePoints(bounds, expectedBounds),
		"Bounding box was not as expected. Result: " + JSON.stringify(bounds)
	);
});
...
test("align: multiple objects grouped, relativeTo is nulls, returns geometry unchanged", (t) => {
	const original = [cube({size: 4, center: [10, 10, 10]}), cube({size: 2, center: [4, 4, 4]})];
	const aligned = align(
		{modes: ["center", "min", "max"], relativeTo: [null, null, null], grouped: true},
		original
	);
	const bounds = measureAggregateBoundingBox(aligned);
	const expectedBounds = [
...
test("align: multiple objects ungrouped, relativeTo is nulls, returns geometry aligned to group bounds", (t) => {
	...
	const aligned = align(
		{modes: ["center", "min", "max"], relativeTo: [null, null, null], grouped: false},
		original
	);
	const bounds = measureAggregateBoundingBox(aligned);
	const expectedBounds = [
