...
const options = (state, i18n) => {
	...
	return html` <section
		class="popup-menu"
		id="options"
		style="visibility:${state.activeTool === "options" ? "visible" : "hidden"}; color:${state.themes
			.themeSettings.secondaryTextColor}">
		<fieldset>
		...
		<fieldset>
			...
			<label
				>${i18n`timeout for generation`}
				<input
					id="solidsTimeout"
					type="number"
					min="0"
					max="200000"
					value=${state.design.solidsTimeOut} />
			</label>
		</fieldset>
