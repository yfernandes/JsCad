...
const reducers = {
	...
	resetDesign: (state, origin) => {
		...
		const design = Object.assign(
			{},
			state.design,
			keep(fieldsToReset, reducers.initialize().design)
		);
		// ugh
	...
	setDesignParameterValues: (state, data) => {
		...
		parameterValues = parameterValues
			? applyParameterDefinitions(parameterValues, state.design.parameterDefinitions)
			: parameterValues;
	...
	setSettings: (state, {data}) => {
		const {vtreeMode, autoReload, instantUpdate, solidsTimeOut} = data;
	...
	requestGeometryRecompute: ({design}, _) =>
		keep(
			["mainPath", "parameterValues", "filesAndFolders", "vtreeMode", "lookup", "lookupCounts"],
			design
		),

	timeoutGeometryRecompute: ({status}, _) => {
		if (status.isBusy) {
			// still computing... we can kill it
