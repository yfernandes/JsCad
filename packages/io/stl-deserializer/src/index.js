...
const deserialize = (options, stl) => {
	...
	const elementFormatterJscad = ({vertices, triangles, normals, colors, index}) =>
		toScript(vertices, triangles, null, colors, index);
	const elementFormatterObject = ({vertices, triangles, normals, colors}) =>
	...
	const result = outputFormatter(
		deserializer(stl, filename, version, elementFormatter),
		addMetaData,
		version,
		filename
	);
...
const deserializeAsciiSTL = (stl, filename, version, elementFormatter) => {
	...
	for (let o = 1; o < objects.length; o++) {
		...
		for (let i = 0; i < match.length; i++) {
			...
			const vpatt =
				/\bfacet\s+normal\s+(\S+)\s+(\S+)\s+(\S+)\s+outer\s+loop\s+vertex\s+(\S+)\s+(\S+)\s+(\S+)\s+vertex\s+(\S+)\s+(\S+)\s+(\S+)\s+vertex\s+(\S+)\s+(\S+)\s+(\S+)\s*/gim;
			...
			for (let k = 0; k < v.length; k++) {
				if (v[k] === "NaN") {
					console.log(
						"bad normal or triangle vertice #" + converted + " " + k + ": '" + v[k] + "', skipped"
					);
					skip++;
				}
			}
		...
		elements.push(elementFormatter({vertices, triangles, colors, index: o}));
	}
