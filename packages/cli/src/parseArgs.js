...
export const parseArgs = (args) => {
	...
	const isValidInputFileFormat = (input) => {
		...
		return inputExtensions.reduce(
			(acc, format) => input.toLowerCase().endsWith("." + format) || acc,
			false
		);
	};
	...
	for (let i = 0; i < args.length; i++) {
		if (args[i] === "-of") {
			...
			// -o <output>
			outputFile = args[++i];
		} else if (args[i] === "-add-metadata") {
			// -metadata true/false
			addMetaData = parseBool(args[++i]);
		} else if (args[i].match(/^--(\w+)=(.*)/)) {
			// params for main()
		...
		} else if (args[i].match(/^-v$/)) {
			// show the version and the environment information
			env();
		} else {
			inputFile = args[i];
