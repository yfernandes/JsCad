...
const getKeyCombos = (options, keyUps$, keyDown$) => {
	...
	if (dropRepeats) {
		keyDown$ = keyDown$.skipRepeatsWith(
			(event, previousEvent) => simpleKey(event) === simpleKey(previousEvent)
		);
	}
...
const makeActions = (sources) => {
	...
	const actionsFromKey$ = most
		.sample(
			({event, compositeKey}, state) => {
				const matchingAction = head(
					state.shortcuts.filter((shortcut) => shortcut.key === compositeKey)
				);
				if (matchingAction) {
	...
	// set shortcuts
	const setShortcuts$ = most
		.mergeArray([
			sources.store.filter((data) => data && data.shortcuts).map((data) => data.shortcuts),
		])
	...
	shortcutCommandUp$.forEach((event) => {
		event.preventDefault();
		event.stopPropagation();
		return false;
	});
	shortcutCommandDown$.forEach((event) => {
		event.preventDefault();
		event.stopPropagation();
		return false;
	});

	const setShortcut$ = most
		.mergeArray([
			shortcutCommandKey$
	...
	const changeTheme$ = most
		.mergeArray([
			sources.dom
				.select("#themeSwitcher")
				.events("change")
				.map((e) => e.target.value),
			sources.store.filter((data) => data && data.themeName).map((data) => data.themeName),
		])
		.startWith("light")
		.map((data) => ({type: "changeTheme", data}));

	const changeLanguage$ = most
		.mergeArray([
			sources.dom
				.select("#languageSwitcher")
				.events("change")
				.map((e) => e.target.value),
			sources.store.filter((data) => data && data.locale).map((data) => data.locale),
		])
		.map((data) => ({type: "changeLanguage", data}));

	const setAvailableLanguages$ = most
		.mergeArray([
			sources.i18n
	...
	const toggleOptions$ = most
		.mergeArray([sources.dom.select("#toggleOptions").events("click")])
		.map((data) => ({type: "toggleOptions", data}));

	// non visual related actions
	const setErrors$ = most
		.mergeArray([sources.solidWorker.filter((event) => "error" in event)])
	...
	const setAppUpdatesAvailable$ = most.mergeArray([
		sources.appUpdates.map((data) => ({type: "setAppUpdatesAvailable", data})),
		sources.appUpdates
