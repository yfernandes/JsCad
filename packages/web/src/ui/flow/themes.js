...
const actions = ({sources}) => {
	const initialize$ = most
		...
		.map((payload) =>
			Object.assign({}, {type: "initializeThemes", sink: "state"}, {state: payload})
		);
	...
	const setTheme$ = most
		.mergeArray([
			...
			sources.store
				.filter(
					(reply) =>
						reply.key === "themes" && reply.type === "read" && reply.data && reply.data.active
				)
	...
	const requestSaveSettings$ = sources.state
		...
		.thru(
			holdUntil(sources.store.filter((reply) => reply.key === "themes" && reply.type === "read"))
		)
