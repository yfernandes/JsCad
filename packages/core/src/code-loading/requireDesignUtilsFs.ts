 ...
 * @param  {} paths
 */
export function getDesignEntryPoint(fs, paths) {
	if (!paths) {
		return;
	}
	...
	if (stats.isFile()) {
		...
		const candidates = entries.filter((entry) =>
			jsMainFiles.concat(jscadMainFiles).includes(entry)
		);
 ...
 */
export function getDesignName(fs, paths) {
	if (!paths) {
		return "undefined";
	}
	const mainPath = toArray(paths)[0];
	const stats = fs.statSync(mainPath);
	if (stats.isFile()) {
		// if main path is a file, find its folder
		const dirName = path.dirname(mainPath);
		return packageNameFromDir(fs, dirName, mainPath);
	} else if (stats.isDirectory()) {
		// if main path is a folder , try to find name from package.json
