...
const reducers = {
	...
	setShortcut: (state, shortcutData) => {
		const alreadyExists = (key) =>
			state.shortcuts.filter((shortcut) => shortcut.key === key).length > 0;

		const shortcuts = state.shortcuts.map((shortcut) => {
			const match =
				shortcut.command === shortcutData.command && shortcut.args === shortcutData.args;
			if (!match) {
				return shortcut;
			} else {
	...
	triggerShortcut: (state, {event, compositeKey}) => {
		const matchingAction = head(
			state.shortcuts.filter((shortcut) => shortcut.key.toLowerCase() === compositeKey)
		);
		if (matchingAction) {
...
const actions = ({sources}) => {
	const initialize$ = most
		...
		.map((payload) =>
			Object.assign({}, {type: "initializeShortcuts", sink: "state"}, {state: payload})
		);
	...
	const setShortcuts$ = most
		.mergeArray([
			sources.store
				.filter(
					(reply) =>
						reply.key === "shortcuts" && reply.type === "read" && reply.data && reply.data.shortcuts
				)
	...
	const keyUps$ = most
		.fromEvent("keyup", sources.dom.element)
		.filter((event) => isKeyEventScopeValid(myKey, event.target))
		.multicast();

	const keyDown$ = most
		.fromEvent("keydown", sources.dom.element)
		.filter((event) => isKeyEventScopeValid(myKey, event.target))
		.multicast();
	...
	const requestSaveSettings$ = sources.state
		...
		.thru(
			holdUntil(sources.store.filter((reply) => reply.key === "shortcuts" && reply.type === "read"))
		)
