...
const designParameters = (state, paramsCallbacktoStream, i18n) => {
	...
	try {
		const {controls} = createParamControls(
			Object.assign({}, parameterDefaults, parameterValues),
			parameterDefinitions,
			paramsCallbacktoStream.callback
		);
		return html`
			<section
				id="params"
				style="visibility:${state.design.parameterDefinitions.length === 0
					? "hidden"
					: "visible"};color:${state.themes.themeSettings.secondaryTextColor}">
				<span id="paramsTable">
					<table>
						${controls}
					</table>
