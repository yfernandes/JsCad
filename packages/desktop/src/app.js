...
titleBar.sink(state$.map((state) => state.appTitle).skipRepeats());
...
electronStore.sink(state$.map(settingsStorage));
...
fs.sink(
	most.mergeArray([
		...
		state$
			...
			.skipRepeatsWith(
				(state, previousState) => JSON.stringify(state) === JSON.stringify(previousState)
			)
			.map(
				({path, enabled}) => ({
					operation: "watch",
...
i18n.sink(
	most.mergeArray([
		...
		state$.take(1).map((data) => ({operation: "getAvailableLanguages", data})),
	])
);

// web worker sink
const solidWorkerBase$ = most
	.mergeArray([
		actions$.setDesignContent$.map((action) => ({
			paramValues: undefined,
			origin: "designContent",
			error: undefined,
		})),
...
solidWorker.sink(
	most
		.sample(
			({origin, paramValues, error}, {design, instantUpdate}) => {
				if (error) {
					return undefined;
				}
				...
				paramValues = paramValues
					? applyParameterDefinitions(paramValues, design.paramDefinitions)
					: paramValues;
			...
			solidWorkerBase$,
			solidWorkerBase$,
			state$.filter((state) => state.design.mainPath !== "").skipRepeats()
		)
		.filter((x) => x !== undefined)
		.map(({source, mainPath, paramValues, options}) => ({
			cmd: "render",
			source,
			mainPath,
...
state$
	...
	.skipRepeatsWith((state, previousState) => {
		...
		const sameSolids =
			state.design.solids.length === previousState.design.solids.length &&
...
const outToDom$ = state$
	.skipRepeatsWith((state, previousState) => {
		const sameParamDefinitions =
			JSON.stringify(state.design.paramDefinitions) ===
			JSON.stringify(previousState.design.paramDefinitions);
		const sameParamValues =
			JSON.stringify(state.design.paramValues) === JSON.stringify(previousState.design.paramValues);

		const sameInstantUpdate = state.instantUpdate === previousState.instantUpdate;

		const sameExportFormats =
			state.exportFormat === previousState.exportFormat &&
		...
		const sameAppUpdates =
			JSON.stringify(state.appUpdates) === JSON.stringify(previousState.appUpdates);
		...
		return (
			...
			sameAppUpdates &&
			sameLocale &&
			sameAvailableLanguages &&
			sameShortcuts
		);
	})
	.combine(
		(state, i18n) => require("./ui/views/main")(state, paramsCallbacktoStream, i18n),
