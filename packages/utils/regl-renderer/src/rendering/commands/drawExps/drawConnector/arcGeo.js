	...
	for (let i = 0, len = numSlices; i <= len; i++) {
		...
		for (let j = 0, lenJ = o.numBands; j < lenJ; j++) {
			...
			if (idxSize === 1) {
				...
				if (i > 0 && j + 1 < lenJ) {
					cells.push([prevSlideIdx + j, curSlideIdx + j]);

					cells.push([curSlideIdx + j + 1, prevSlideIdx + j + 1]);

					if (!o.drawOutline) {
						cells.push([curSlideIdx + j, curSlideIdx + j + 1]);
					}
				}
			} else if (idxSize === 3) {
				if (i > 0 && j + 1 < lenJ) {
					cells.push([curSlideIdx + j, prevSlideIdx + j + 1, prevSlideIdx + j]);

					cells.push([curSlideIdx + j, curSlideIdx + j + 1, prevSlideIdx + j + 1]);
				}
			}
		}
	}
