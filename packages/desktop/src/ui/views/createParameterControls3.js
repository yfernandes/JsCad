...
const createParamControls = (prevParamValues = {}, paramDefinitions, rebuildSolid) => {
	...
	const controls = paramDefinitions.map((paramDefinition) => {
		...
		const subItems = subControls.map(
			(control) => html`<div>${control} ${"label" in control ? control.label : ""}</div>`
		);
...
const createChoiceControl = (definition, prevValue) => {
	if (!("values" in definition)) {
		throw new Error(
			"Definition of choice parameter (" + definition.name + ") should include a 'values' parameter"
		);
	}
	...
	const options = captions.map((caption, index) => {
		...
		return html`<option value=${value} selected=${selected}>${caption}</option>`;
	});
...
const createControl = (definition, prevValue) => {
	...
	typeData.required.forEach((requiredField) => {
		if (!definitionFields.includes(requiredField)) {
			throw new Error(
				`Parameter definition for "${definition.name}" must include a "${requiredField}" parameter`
			);
		}
	});
    ...
    type=${typeData.control} value=${controlValue} checked=${
			"checked" in definition ? controlValue : ""
		}>
  </input>`;
		...
		if (controlInstance.type === definition.type) {
			for (j = 0; j < controlInstance.required.length; j++) {
				...
				if (paramName in definition) {
					...
					throw new Error(
						"Parameter definition (" + definition + ") must include a '" + paramName + "' parameter"
					);
				}
			}
			break;
