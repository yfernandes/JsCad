...
test("deserialize : instantiate svg (rect) to objects", (t) => {
  ...
  <rect x="40" y="20" width="250" height="250" transform="translate(60 50) scale(2 3)"/>
	...
	t.deepEqual(measureBoundingBox(shape), [
		[14, -86, 0],
		[64, -11, 0],
	]);
...
`;
	...
	observed = deserialize(
		{output: "geometry", target: "path", addMetaData: false, segments: 16},
		sourceSvg
	);
	t.is(observed.length, 1);
	shape = observed[0];
...
test("deserialize : instantiate svg (path: arc) to objects", (t) => {
  ...
  <path d="M 230 230 A 45 45 0 1 1 275 275 L 275 230 Z" transform="translate(180 80) rotate(45)"/>
	...
	t.deepEqual(measureBoundingBox(shape), [
		[64.91110599999999, -77.611105, 0],
		[90.21850570104527, -52.30370029895471, 0],
	]);
	shape = observed[1];
	t.is(shape.points.length, 15); // segments double on a 3/4 circle
	t.deepEqual(measureBoundingBox(shape), [
		[50.799996, -136.03302387090216, 0],
		[72.27222493929787, -110.6793647936299, 0],
	]);
});
...
`;
	...
	let observed = deserialize(
		{filename: "inkscape", output: "geometry", target: "geom2", addMetaData: false},
		sourceSvg
	);
	t.is(observed.length, 2);
	let shape = observed[0];
...
test("deserialize : translate svg with simple defs to script", (t) => {
  ...
  <g id='heurekasvg' transform='scale(0.29333) translate(-170,36) '>
	...
	const observed = deserialize(
		{output: "geometry", target: "geom2", addMetaData: false},
		sourceSvg
	);
	t.is(observed.length, 1);
