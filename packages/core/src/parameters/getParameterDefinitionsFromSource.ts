...
const extractTextFromComment = (c, lineNum) => {
	...
	if (prefix === "/*") {
		if (c.substring(c.length - 2) !== "*/")
			throw new EvalError(
				`Multi-line comments not supported in parsed parameter definitions, line:${lineNum}`,
				"code",
				lineNum
			);
...
export function parseComment(comment, lineNum, paramName) {
	...
	if (idx !== -1) {
		try {
			...
			throw new EvalError(
				`${e.message}, parameter:${paramName}, line:${lineNum}: ${comment.substring(idx)}`,
				"code",
				lineNum
			);
		}
	...
	if (idx === -1) {
		...
		if (initial === "true" || initial === "false") {
			...
			try {
				...
				throw new EvalError(
					`Error in the initial value definition for ${code}  ${e.message}, line:${line}`,
					"code",
					line
				);
			}
