...
const {translate, rotate, scale, mirror, hull, chain_hull, contract} =
	require("@jscad/csg/api").transformations;
	...
	switch (node.type) {
		...
		case "measureArea":
			...
			result = operands.reduce((acc, csg) => {
				const tmpArea = csg
					.toTriangles()
					.reduce((accSub, triPoly) => accSub + triPoly.getTetraFeatures(["area"]), 0);
				return acc + tmpArea;
			}, 0);
			break;
		default:
