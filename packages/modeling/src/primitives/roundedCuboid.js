...
const createCorners = (center, size, radius, segments, slice, positive) => {
	...
	const corner0 = vec3.add(vec3.create(), center, [
		size[0] - radius,
		size[1] - radius,
		layerOffset,
	]);
	const corner1 = vec3.add(vec3.create(), center, [
		radius - size[0],
		size[1] - radius,
		layerOffset,
	]);
	const corner2 = vec3.add(vec3.create(), center, [
		radius - size[0],
		radius - size[1],
		layerOffset,
	]);
	const corner3 = vec3.add(vec3.create(), center, [
		size[0] - radius,
		radius - size[1],
		layerOffset,
	]);
...
const stitchSides = (bottomCorners, topCorners) => {
	...
	for (let i = 0; i < topVertices.length; i++) {
		...
		polygons.push(
			poly3.create([bottomVertices[i], bottomVertices[j], topVertices[j], topVertices[i]])
		);
	}
	return polygons;
};
 ...
 */
export function roundedCuboid(options) {
	const defaults = {
		center: [0, 0, 0],
		size: [2, 2, 2],
		roundRadius: 0.2,
