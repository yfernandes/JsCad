...
const tube = (bezierControlPoints) => {
	...
	tubeSlice = slice.transform(
		rotationMatrixFromVectors(maths.vec3.clone([0, 0, 1]), bezierDelta),
		tubeSlice
	);
	...
	return extrusions.extrudeFromSlices(
		{
			...
			callback: function (progress, count, base) {
				...
				const rotationMatrix = rotationMatrixFromVectors(
					bezierDelta,
				...
				return slice.transform(
					maths.mat4.multiply(translationMatrix, translationMatrix, rotationMatrix),
					base
				);
			},
		},
