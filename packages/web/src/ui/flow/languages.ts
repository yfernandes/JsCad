...
const actions = ({sources}) => {
	...
	const initialize$ = most
		...
		.map((payload) =>
			Object.assign({}, {type: "initializeLanguages", sink: "state"}, {state: payload})
		);
	...
	const requestGetAvailableLanguages$ = initialize$.map((_) => ({
	...
	const requestGetDefaultLanguage$ = requestGetAvailableLanguages$.map((_) => ({
	...
	const setAvailableLanguages$ = most
		...
		.map((payload) =>
			Object.assign({}, {type: "setAvailableLanguages", sink: "state"}, {state: payload})
		)
		.multicast();
	...
	const requestSaveSettings$ = sources.state
		...
		.thru(
			holdUntil(sources.store.filter((reply) => reply.key === "languages" && reply.type === "read"))
		)
