...
test("line3: fromPointAndDirection() should return a new line3 with correct values", (t) => {
	let obs = Line3.fromPointAndDirection(Line3.create(), [0, 0, 0], [1, 0, 0]);
	let pnt = obs[0];
	let dir = obs[1];
	t.true(compareVectors(pnt, [0, 0, 0]));
	t.true(compareVectors(dir, [1, 0, 0]));

	obs = Line3.fromPointAndDirection(Line3.create(), [1, 0, 0], [0, 2, 0]);
	pnt = obs[0];
	dir = obs[1];
	t.true(compareVectors(pnt, [1, 0, 0]));
	t.true(compareVectors(dir, [0, 1, 0]));

	obs = Line3.fromPointAndDirection(Line3.create(), [0, 1, 0], [3, 0, 0]);
	pnt = obs[0];
	dir = obs[1];
	t.true(compareVectors(pnt, [0, 1, 0]));
	t.true(compareVectors(dir, [1, 0, 0]));

	obs = Line3.fromPointAndDirection(Line3.create(), [0, 0, 1], [0, 0, 4]);
	pnt = obs[0];
	dir = obs[1];
	...
	obs = Line3.fromPointAndDirection(Line3.create(), [0, 5, 0], [0, 0, 0]);
	pnt = obs[0];
	dir = obs[1];
