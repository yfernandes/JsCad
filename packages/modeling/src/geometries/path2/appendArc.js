...
export function appendArc(options, geometry) {
	...
	let {endpoint, radius, xaxisRotation, clockwise, large, segments} = Object.assign(
		{},
		defaults,
		options
	);
	...
	if (xRadius === 0 || yRadius === 0) {
		...
		// round to precision in order to have determinate calculations
		const x =
			Math.round((cosPhi * minusHalfDistance[0] + sinPhi * minusHalfDistance[1]) * decimals) /
			decimals;
		const y =
			Math.round((-sinPhi * minusHalfDistance[0] + cosPhi * minusHalfDistance[1]) * decimals) /
			decimals;
