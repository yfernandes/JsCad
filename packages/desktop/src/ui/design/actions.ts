...
const actions = (sources) => {
	...
	const setDesignPath$ = designPath$.map((data) => ({type: "setDesignPath", data})).delay(1);
	...
	const updateDesignFromParams$ = most
		.mergeArray([
			sources.dom
				...
				.map(() => {
					const controls = Array.from(
						document.getElementById("paramsTable").getElementsByTagName("input")
					).concat(
						Array.from(document.getElementById("paramsTable").getElementsByTagName("select"))
					);
					const paramValues = getParameterValuesFromUIControls(controls);
					return {paramValues, origin: "manualUpdate"};
				})
				.multicast(),
			sources.paramChanges.multicast().map((_controls) => {
				// FIXME: clunky
				try {
					const controls = Array.from(
						document.getElementById("paramsTable").getElementsByTagName("input")
	...
	const timeOutDesignGeneration$ = most
		.never()
		/* designPath$
    sources.state$
    .delay(60000) */
