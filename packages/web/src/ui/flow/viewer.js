...
const actions = ({sources}) => {
	const initializeViewer$ = most
		.just({})
		.thru(withLatestFrom(reducers.initialize, sources.state))
		.map((payload) =>
			Object.assign({}, {type: "initializeViewer", sink: "state"}, {state: payload})
		);

	const toggleGrid$ = most
		.mergeArray([
			sources.dom
				.select("#toggleGrid")
				.events("click")
	...
	const toggleAutoRotate$ = most
		.mergeArray([
			sources.dom
				.select("#toggleAutoRotate")
				.events("click")
				.map((e) => e.target.checked),
		])
	...
	const toggleAutoZoom$ = most
		.mergeArray([
			sources.dom
				.select("#toggleAutoZoom")
				.events("click")
				.map((e) => e.target.checked),
		])
	...
	const otherViewerActions$ = sources.actions
		...
		.thru(
			withLatestFrom((state, action) => reducers[action.type](state, action.data), sources.state)
		)
