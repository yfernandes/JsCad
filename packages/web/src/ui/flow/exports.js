...
const reducers = {
	...
	setExportFormat: (state, exportFormat) => {
		const io = Object.assign(
			{},
			state.io,
			exportFilePathFromFormatAndDesign(state.design, exportFormat),
			{exportFormat}
		);
		return {io};
	},
...
const actions = ({sources}) => {
	const initializeExports$ = most
		.just({})
		.thru(withLatestFrom(reducers.initialize, sources.state))
		.map((payload) =>
			Object.assign({}, {type: "initializeExports", sink: "state"}, {state: payload})
		);

	const changeExportFormat$ = sources.dom
		.select("#exportFormats")
		.events("change")
		.map((e) => e.target.value)
		.thru(withLatestFrom(reducers.setExportFormat, sources.state))
		.map((payload) =>
			Object.assign({}, {type: "changeExportFormat", sink: "state"}, {state: payload})
		)
		.multicast();

	const requestExport$ = sources.dom
		.select("#exportBtn")
		.events("click")
