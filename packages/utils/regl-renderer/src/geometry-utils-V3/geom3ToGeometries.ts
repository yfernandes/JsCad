	...
	for (let i = 0; i < polygons.length; i++) {
		...
		for (let j = 0; j < vertices.length; j++) {
			...
			if (smoothLighting) {
				...
				const existingTupple = fuzyNormalAndPositionLookup(
				...
				if (!existingTupple) {
					...
					if (!existingPositing) {
						...
						normalPositionLookup[candidateTupple.position] =
							normalPositionLookup[candidateTupple.position].concat(itemToAdd);
					}
					index = tupplesIndex;
					if (faceColor) {
						colors.push(faceColor);
...
const areNormalsSimilar = (normal, otherNormal, threshold) =>
	vec3.distance(normal, otherNormal) <= threshold;
