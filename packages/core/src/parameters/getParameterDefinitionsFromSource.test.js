...
test("comment with data", (t) => {
	t.deepEqual(parseComment('/* Password {type:"password"}*/'), {
		caption: "Password",
...
test("param checkbox", (t) => {
	...
	t.deepEqual(parseDef("bigorsmall=false,"), {
		name: "bigorsmall",
		type: "checkbox",
		checked: false,
	});
...
test("param checkbox with initial value", (t) => {
	testBothDir(t, '// Big? {type:"checkbox"}', "bigorsmall=20,", {
		name: "bigorsmall",
		type: "checkbox",
		checked: true,
		initial: 20,
...
test("param color", (t) => {
	testBothDir(t, '// Color? {type:"color"}', "color='#FFB431'", {
		name: "color",
		type: "color",
		initial: "#FFB431",
		caption: "Color?",
...
test("param date", (t) => {
	testBothDir(t, '// Birthday? {type:"date"}', "birthday", {
		name: "birthday",
		type: "date",
		caption: "Birthday?",
	});
...
test("param email", (t) => {
	testBothDir(t, '// Email Address? {type:"email"}', "address", {
		name: "address",
		type: "email",
...
test("param password", (t) => {
	testBothDir(t, '// Secret? {type:"password"}', "password", {
		name: "password",
		type: "password",
		caption: "Secret?",
	});
...
test("param slider", (t) => {
	testBothDir(t, '// How many? {type:"slider", min:2, max:10}', "count", {
		name: "count",
		type: "slider",
		min: 2,
		max: 10,
...
test("param slider2", (t) => {
	testBothDir(t, '// How many? {type:"slider", min:2, max:10, step:2}', "count", {
		name: "count",
		type: "slider",
		min: 2,
		max: 10,
...
test("param url", (t) => {
	testBothDir(t, '// Web page URL? {type:"url"}', "webpage", {
		name: "webpage",
		type: "url",
...
const sampleParamsWithHints = [
	{
		...
		type: "int",
		initial: 145,
		hint: "Width of the complete model\nIncluding other stuff",
	},
	{
		name: "height",
		caption: "height",
		type: "int",
		initial: 100,
		hint: "Height of the complete model",
	},

	{
		name: "_group_1",
		caption: "Wall thickness :group1",
		type: "group",
		hint: "Extra description of the group so group name can stay short",
		initial: "closed",
	},
	{name: "thickOut", caption: "Vertical outside", type: "number", initial: 0.8},
];
...
test("last param brackets", (t) => {
	t.deepEqual(
		getParameterDefinitionsFromSource(`function main({//@jscad-params
  width=14})`),
		sampleParams2
	);
});
  ...
  width=14})`),
		{
			...
			message: "Multi-line comments not supported in parsed parameter definitions, line:2",
		}
	);
});
...
test("single line /* ...*/ ok", (t) => {
  ...
  /* group */
  width=14})`),
		[{type: "group", caption: "group", name: "_group_1"}, ...sampleParams2]
	);
});
