...
const makeDomSideEffect = ({targetEl}) => {
	...
	const domSink = (targetEl, outToDom$) => {
		...
		const firstRender$ = outToDom$.take(1).map((_tree) => {
			tree = _tree;
			targetEl.appendChild(tree);
		});
		const otherRenders$ = outToDom$.skip(1).map((newTree) => {
			morph(tree, newTree);
		});

		const domRenderRequest$ = most.mergeArray([firstRender$, otherRenders$]).multicast();
