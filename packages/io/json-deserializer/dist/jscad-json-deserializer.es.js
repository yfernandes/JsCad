...
const flatten = (arr) =>
	...
	deserialize = (options, input) => {
		...
		return (
			(objects = flatten(
				((array = objects), Array.isArray(array) ? array : null == array ? [] : [array])
			)),
			"script" === options.output ? translate(options, objects) : objects
		);
	},
	translate = (options, objects) => {
		const {addMetaData: addMetaData, filename: filename, version: version} = options;
		let script = addMetaData
			? `//\n// Produced by JSCAD IO Library : JSON Deserializer (${version})\n// date: ${new Date()}\n// source: ${filename}\n//\n`
			: "";
		return (
			(script += `\nconst { geometries } = require('@jscad/modeling')\n\nconst main = () => {\n  const objects = [${translateToList(
				objects
			)} ]\n  return objects\n}\n\n${translateToObjects(objects)}\n\nmodule.exports = { main }\n`),
			script
		);
	},
	translateToList = (objects) =>
	...
	translateToObject = (object, index) => `const json${index} = ${JSON.stringify(object)}\n`,
